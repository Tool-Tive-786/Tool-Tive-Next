...
export default function ToolCard({ title, description, icon, tags, category, href }: ToolCardProps) {
  return (
    <li style={{ listStyle: 'none', display: 'flex' }}>
      <div className="tool-card" data-category={category} itemProp="itemListElement" itemScope itemType="https://schema.org/SoftwareApplication" style={{ width: '100%' }}>
        <div className="tool-card-body">
          <div className="tool-icon-wrap" aria-hidden="true" dangerouslySetInnerHTML={{ __html: icon }}>
          </div>
          <h3 itemProp="name">{title}</h3>
          <p itemProp="description" className="line-clamp-2">{description}</p>
        ...
        <footer className="tool-card-footer">
          ...
          <Link href={href} className="tool-cta" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
